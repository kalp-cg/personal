...
import { FaHome, FaUser, FaGraduationCap, FaBriefcase, FaEnvelope, FaRss } from 'react-icons/fa';
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`fixed top-0 left-0 w-20 md:w-72 h-full z-30 hidden md:flex flex-col items-center md:items-start pt-20 pb-10
          ${theme === 'dark' ? 'bg-[#222]' : 'bg-white shadow-lg'}`}
      >
        {/* Profile Section - only visible on wider screens */}
        <div className="hidden md:flex flex-col items-center justify-center w-full mb-12 px-10">
        ...
        {/* Navigation */}
        <div className="flex flex-col items-center md:items-stretch justify-center gap-6 w-full px-4">
          {menuItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              ...
              className={({ isActive }) => 
                `flex flex-col md:flex-row items-center md:px-6 py-2 rounded-lg transition-all
                 ${isActive ? 'text-primary' : theme === 'dark' ? 'text-white' : 'text-gray-600'}
            ...
            >
              <motion.span 
                ...
                className="text-xl mb-1 md:mb-0 md:mr-3"
              >
                {item.icon}
              </motion.span>
              <span className="text-xs md:text-base font-medium">{item.text}</span>
            </NavLink>
          ))}
        </div>
        
        {/* Social Media Links - only visible on wider screens */}
        <div className="hidden md:flex mt-auto w-full px-10">
          <div className="flex justify-center gap-4 w-full pt-6 border-t border-gray-200 dark:border-gray-700">
      ...
      <motion.div 
        ...
        className={`fixed inset-0 bg-black z-40 transition-opacity duration-300 ${isMobileNavOpen ? 'block' : 'hidden'}`}
      ...
      <motion.div
        ...
        className={`fixed top-0 left-0 w-64 h-full z-50 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} shadow-xl md:hidden`}
      >
        <div className="p-5">
          <div className="flex justify-between items-center mb-10">
            ...
            <button onClick={() => setIsMobileNavOpen(false)} className="text-gray-500">
              ...
              <NavLink
                ...
                className={({ isActive }) => 
                  `flex items-center gap-3 py-2 px-4 rounded-lg transition-all
                   ${isActive ? 'text-primary bg-primary/10' : theme === 'dark' ? 'text-white' : 'text-gray-600'}`
                }
              >
                <span>{item.icon}</span>
                <span>{item.text}</span>
